...
class  AccessAccount:
    ...
    def fetch(ip):
        ...
        with requests.session() as req_session:
            q = req_session.post(url=login_url, json=login_data)
            url = f"{XC_URL}/prod-api/cmdb/account_manage_get_logger_pwd?ip={ip}"
            with req_session.get(url=url) as response:
                result = response.json()
                if result["res"]:
                    user = result["res"][0]["account_name"]
                    password = result["res"][0]["pwd"]
                    return {
                        "user":user,
                        "pwd":password
                    }
    ...
    def authentication(self):
        ...
        xtoken = self.get_cookie("Admin-Token")
        if referer and referer.startswith("https://xc.yto.net.cn/vue/") and xtoken:
            return  True
    ...
    @staticmethod
    def empower(req,host,account):
        xtoken = req.get_cookie("Admin-Token")
        xc_url = f'{XC_URL}/prod-api/user/user_info?token={xtoken}'
        res = requests.get(xc_url, timeout=5)
        if res.status_code != 200:
            return  False
        result = res.json()
        if result['code'] == 20000:
            user_info_xc = result["user_info"]
            user_uuid = user_info_xc['card']
            if user_uuid in ["02222970","02171824"]:
                logging.info(f"特权用户登录：{user_uuid} : {user_info_xc}")
                return True


        # 继续判断
        auth_url = "{}/prod-api/cmdb/is_authorization/?ip={}&account={}".format(XC_URL,host,account)
        xc_headers = {"Cookie":req.request.headers.get("Cookie")}
        response = requests.get(auth_url,headers = xc_headers, timeout = 20)
        print(response.text)
        result = response.json()
        #{"flag": 0}
    ...
    @staticmethod
    def fetch_deploy_account(ip,account):
        # 根据传入的发布账号 获取到密码
        login_data = {'username': 'admin', 'password': XC_PASSPORT, "develop_key": "xc"}
        login_url = '{}/prod-api/user/login'.format(XC_URL)
        with requests.session() as req_session:
...
class IndexHandler(MixinHandler, tornado.web.RequestHandler):
    ...
    def get_args(self):
        ...
        # 创建ssh客户端,向前端返回唯一的uuid

        result = AccessAccount.fetch_deploy_account(hostname, username)
        if not result:
            raise InvalidValueError(f"cmdb中不存在该机器的{username}账号")
        # 根据账号和密码获取到
        password = result["pwd"]

        if not AccessAccount.empower(self,hostname,username):
            raise InvalidValueError(f"{hostname}的{username}没有授权给你,请提交密码申请单申请权限！")
    ...
    def get(self):
        ok = AccessAccount.authentication(self)
        if not ok:
            self.set_status(403)
            self.finish(
                self.html.format(code=self._status_code, reason=self._reason)
    ...
    def post(self):
        ...
        if len(clients) >= options.maxconn:
            raise tornado.web.HTTPError(403, 'Too many live connections.')
...
class WsockHandler(MixinHandler, tornado.websocket.WebSocketHandler):
    ...
    def open(self):
        ...
        try:
            ...
            # else:
            #     self.close(reason='Websocket authentication failed.')
            worker = clients.get(worker_id,None)
            if worker:
                #clients[worker_id] = None
