...
def clear_worker(worker, clients):
    ...
    assert worker.id in clients
    clients.pop(worker.id)
...
class Worker(object):
    ...
    def expire_judge(self):
        expire_time = self.create_time + datetime.timedelta(minutes=30)
        now = datetime.datetime.now()
        if now > expire_time:
            self.data_to_dst.append("\x03")
            self.on_write()
            self.data_to_dst.append("logout\n")
            self.on_write()
